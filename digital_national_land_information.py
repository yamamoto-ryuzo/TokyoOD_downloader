...
try:
    ...
    TokyoOD_functions.clean_work_folder(work_folder_path)
    ### geoファイルの取得及び各県のデータを１ファイルに結合
    TokyoOD_functions.geo_download("input_list/digital_national_land_information_url_list.txt")
    ...
    TokyoOD_functions.unify_crs_in_folder(folder_path, target_epsg)

### フォルダ内のすべてのSHPにジオメトリの経度X、緯度Y座標属性値を追加
    TokyoOD_functions.add_coordinates_to_shapefiles(folder_path)

    TokyoOD_functions.convert_shp_to_gpkg(folder_path,folder_path)

    ### SHP属性名コードを日本語属性名に変更
    # 対象のSHPファイルが含まれるフォルダのパス
    # UTF-8　で作成のこと
    csv_file_path = 'input_list/国土数値情報コード名称変換.csv'
    TokyoOD_functions.replace_attributes(folder_path,csv_file_path)

    ### フォルダ内のすべてのジオメトリをPOINTに変更
    gpkg_folder_path = 'work/other_files_folder'
    TokyoOD_functions.convert_geometries_to_points(gpkg_folder_path)

    ### フォルダ内のGEOファイルを結合
    folder_path = 'work/other_files_folder'
    gpkg_file_path = 'result/digital_national_land_information.gpkg'
    TokyoOD_functions.merge_geopackages(folder_path, gpkg_file_path)
    ...
    TokyoOD_functions.change_new_city_code(gpkg_file_path, csv_file_path)
    # [国土数値情報用行政区域コード]から[所在地_都道府県][所在地_市区町村]を追加
    TokyoOD_functions.add_city_name(gpkg_file_path, csv_file_path)

    ### 他の形式も作成
    csv_file_path = 'result/digital_national_land_information.csv'
    TokyoOD_functions.convert_format(gpkg_file_path, csv_file_path, input_format='gpkg', output_format='csv')
# エラー処理
